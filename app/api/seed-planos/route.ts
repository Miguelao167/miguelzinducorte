...
// GET ou POST - Cria os 4 planos padrão se não existirem
async function seedPlanos() {
  ...
  return { criados, jaExistentes, total: todos.length, planos: todos.map(p => ({ id: p.id, nome: p.nome, preco: p.preco, numeroCortes: p.numeroCortes })) }
}

export async function GET() {
  try {
    return NextResponse.json(await seedPlanos())
  } catch (error: any) {
